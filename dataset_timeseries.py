...
class TimeseriesData(object):
    ...
    def df_convert_dtypes(self, convert_from, convert_to):
    ...
    def analyze_feature_importance(self, method = 'tree', n_features=10, random_state=42):
        ...
        if method == 'information':
            ...
            top_features = pd.Series([X.columns[i] for i in np.argsort(mi_scores)[-n_features:]])  # Select top 5 features
    ...
    def plot_seasonal_decomposition(self):
        ...
        res = sm.tsa.seasonal_decompose(pd.Series(self.features[self.target_col].values, index = self.features[self.time_index]), model='multiplicative')
    ...
    def get_data(self):
        return self.features.drop(columns = [self.target_col]).values, self.target.flatten()
    
    def convert_time_index_from_string_to_datetime(self):
        self.data[self.time_index] = pd.to_datetime(self.data[self.time_index], utc=True, infer_datetime_format=True)
        self.features[self.time_index] = pd.to_datetime(self.features[self.time_index], utc=True, infer_datetime_format=True)

    def _check_datetime(self):
        if not pd.api.types.is_datetime64_any_dtype(self.features[self.time_index]):
            raise TypeError(f"Time index must be of datetime type. Please try calling self.convert_time_index_from_string_to_datetime().")
    ...
    def create_weekday_feature(self,add_holidays = True):
        ...
        weekdays = (self.features[self.time_index].dt.day_of_week.astype(int).values > 4).astype(int)
    ...
    def create_lag_feature(self, feature,lag, target = False):
        if target:
            self.features['target'+'lag'+str(lag)] = self.data[self.target_col].shift(lag)
        else:
            self.features[feature+'lag'+str(lag)] = self.features[feature].shift(lag)

    def create_rolling_mean_feature(self, feature,window,lag = 0, target = False):
        if target:
            self.features['target'+'rolling_mean'+str(window)+'lag'+str(lag)] = self.data[self.target_col].shift(lag).rolling(window=window).mean()
        else:
            self.features[feature+'rolling_mean'+str(window)+'lag'+str(lag)] = self.data[self.target_col].shift(lag).rolling(window=window).mean()

    def create_rolling_min_feature(self, feature,window,lag = 0, target = False):
        if target:
            self.features['target'+'rolling_min'+str(window)+'lag'+str(lag)] = self.data[self.target_col].shift(lag).rolling(window=window).min()
        else:
            self.features[feature+'rolling_min'+str(window)+'lag'+str(lag)] = self.data[self.target_col].shift(lag).rolling(window=window).min()

    def create_rolling_max_feature(self, feature,window,lag = 0, target = False):
        if target:
            self.features['target'+'rolling_max'+str(window)+'lag'+str(lag)] = self.data[self.target_col].shift(lag).rolling(window=window).max()
        else:
            self.features[feature+'rolling_max'+str(window)+'lag'+str(lag)] = self.data[self.target_col].shift(lag).rolling(window=window).max()

    def create_rolling_std_feature(self, feature,window,lag = 0, target = False):
        if target:
            self.features['target'+'rolling_std'+str(window)+'lag'+str(lag)] = self.data[self.target_col].shift(lag).rolling(window=window).std()
        else:
            self.features[feature+'rolling_std'+str(window)+'lag'+str(lag)] = self.data[self.target_col].shift(lag).rolling(window=window).std()
    ...
    def create_fft_feature(self, window=48,lag = 0, target = False):
        ...
        results = pd.DataFrame([rolling_fft(df_) for df_ in self.features[self.target_col].rolling(window)])
    ...
    def create_seasonal_feature(self,lag=24):
        ...
        seasonal_data = sm.tsa.seasonal_decompose(pd.Series(vals,index = self.data[self.time_index]), model='additive')
    ...
    def replace_cyclic_feature_with_rotary(self,feature,period):
        self.features[feature+'x'] = np.cos(((2.0 * np.pi) / period) * self.features[feature].astype(float))
        self.features[feature+'y'] = np.sin(((2.0 * np.pi) / period) * self.features[feature].astype(float))
    ...
    def return_training_data(self,n_steps_ahead_to_predict = 0,
                            train_size = .7, 
                            validation_size = .15, rng = False,scale = True):

        if n_steps_ahead_to_predict < 0:
            raise ValueError(f"Value of n_steps_ahead_to_predict must be positive.")
        ...
        if n_steps_ahead_to_predict == 0:
            ...
            X = training_data.drop(columns = [self.time_index,self.target_col]).iloc[:-n_steps_ahead_to_predict,:].values
            y = training_data[self.target_col].shift(-n_steps_ahead_to_predict).values[:-n_steps_ahead_to_predict]

        if rng:
            y = pd.DataFrame({f'step_{i}': training_data[self.target_col].shift(-i) for i in range(n_steps_ahead_to_predict)}).values[:-n_steps_ahead_to_predict]
    ...
    def toSequence(self, sequenceSize, steps_to_predict=1):
        ...
        for i in range(len(data) - sequenceSize - steps_to_predict+1):
            ...
            target = data[i + sequenceSize : i+sequenceSize + steps_to_predict,target_index]
        ...
        return torch.tensor(X, dtype=torch.float).reshape(-1, sequenceSize, data.shape[1]).numpy(), torch.tensor(y, dtype=torch.float).reshape(-1, steps_to_predict).numpy()

    def return_training_data_recurrent(self,sequenceSize,n_steps_ahead_to_predict,train_size = .85, validation_size = .1):
...
class GridmaticTimeseries(TimeseriesData):
    def __init__(self,filepath,target_col,time_index,):
    ...
    def clean_and_implement_features(self, lag_range = 72,convert_to_cyclic = True):
        ...
        for col in self.features:
            if col != self.time_index and (col == self.target_col or "trend" in col):
                self.create_rolling_mean_feature(feature=col,target = False,lag = 1, window = 24 * 31)
                # self.create_rolling_mean_feature(feature=col,target = False,lag = 1, window = 24 * 14)
                self.create_rolling_mean_feature(feature=col,target = False,lag = 1, window = 24 * 7)
                self.create_rolling_mean_feature(feature=col,target = False,lag = 1, window = 24 * 3)
                self.create_rolling_mean_feature(feature=col,target = False,lag = 1, window = 24 )
