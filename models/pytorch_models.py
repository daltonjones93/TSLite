...
class denseFFN(nn.Module):
    def __init__(self,input_size,output_size, n_hidden_units, n_layers, dropout_param =.05,**kwargs):
        super().__init__()
...
class SimpleAttention(nn.Module):
    ...
    def forward(
        ...
        if self.is_causal:
            ...
            mask = mask.expand(bsz, self.num_heads, -1, -1)  # Shape: (batch_size, num_heads, seq_length, seq_length)
        ...
        for epoch in range(self.epochs):
            ...
            if isinstance(X_test,torch.Tensor):
                if epoch % 10 == 0:
                    ...
                    mse_train = mean_squared_error(y_pred_train.detach().numpy().flatten(),y_test.detach().numpy().flatten())
...
class TransformerModel(nn.Module):
    def __init__(self,input_size,output_size, hidden_size,n_layers,dropout_param = .05,max_len = 2000,**kwargs):
        super().__init__()
        ...
        for i in range(n_layers):
            ...
            self.ffn_layers.append(denseFFN(hidden_size,hidden_size,hidden_size * 2, 1))
    ...
    def forward(self,x):
        if x.shape[1] > self.max_len:
            raise ValueError("x is too long for the model, try to shorten the sequence of inputs")
        x = self.ffn0(x)
...
class LSTMModel(nn.Module):
    def __init__(self,input_size,output_size, hidden_size,n_layers,dropout_param = .05,**kwargs):
        super().__init__()
...
def grid_search(xTrain, yTrain, xVal, yVal, param_grid,model_type = 'transformer', output_json="best_params.json"):
    best_params = None
    ...
    for combination in all_combinations:
        ...
        with torch.no_grad():
            ...
            val_score = mean_squared_error(yVal.numpy(), ypred)  # Use your chosen metric here
